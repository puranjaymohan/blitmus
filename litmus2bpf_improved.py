...
statement: decl | write_once | read_once | plain_store | plain_load | fence | store_release | load_acquire | imm_assign | arith_assign | if_stmt | macro
...
condition: VAR_NAME | VAR_NAME "==" NUMBER | VAR_NAME "!=" NUMBER
...
class LitmusTransformer(Transformer):
    ...
    def if_stmt(self, items): return IfStatement(items[0], items[1] if isinstance(items[1], list) else [items[1]])
