...
#define smp_mb()						\
        ({							\
                volatile __u64 __val;				\
		__val = __sync_fetch_and_add(&__val, 0);	\
        })
...
struct {
    ...
    volatile __u64 r3[10000];  // For P3_r0
    volatile __u64 r4[10000];  // For P3_r1
} shared;
...
int handle_tp1(void *ctx)
{
	...
	bpf_for (i, 0, 10000) {
		barrier_wait(0, i);
		smp_store_release(&shared.x[i], 1);
	}
	smp_mb();
	return 0;
}
...
int handle_tp2(void *ctx)
{
	...
	bpf_for (i, 0, 10000) {
		barrier_wait(1, i);
		shared.r1[i] = smp_load_acquire(&shared.x[i]);
