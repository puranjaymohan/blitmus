...
#include "z6_0_pooncerelease_poacquirerelease_fencembonceonce.skel.h"
#define THREADS 3
#define TEST_NAME z6_0_pooncerelease_poacquirerelease_fencembonceonce
#define TEST_NAME_PRINT "Z6_0+pooncerelease+poacquirerelease+fencembonceonce"
#define EXISTS_CLAUSE "1:r0=1 /\\ z=2 /\\ 2:r1=0"
unsigned int states[10][10][10] = {0};
...
static void check_cond (STRUCT_NAME(TEST_NAME) *skel,
	...
	int p2_r1 = skel->bss->shared.r2[c];
	int var_z = skel->bss->shared.z[c];
	// Check if this iteration matches the exists clause
	states[p1_r0][p2_r1][var_z]++;
	if (p1_r0 == 1 && p2_r1 == 0 && var_z == 2) {
			*matches += 1;
			expected_state_p = &states[p1_r0][p2_r1][var_z];
	} else {
			*non_matches += 1;
	}
}
const char* var_names[] = {"p1_r0", "p2_r1", "var_z"};
