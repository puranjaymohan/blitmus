...
const char *cond_vars_str[3] = {"1:r0", "2:r1", "z"};
...
static void check_cond (STRUCT_NAME(TEST_NAME) *skel, unsigned long long *matches,
	...
	unsigned long long z = skel->bss->shared.z[c];
	...
	key_values[1] = skel->bss->shared.P2_r1[c];
	key_values[2] = skel->bss->shared.z[c];

        // Check if this iteration matches the exists clause
        if (((P1_r0 == 1) && (z == 2) && (P2_r1 == 0))) {
                *matches += 1;
                target = true;
        } else {
                *non_matches += 1;
