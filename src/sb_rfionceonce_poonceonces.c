...
static void check_cond (STRUCT_NAME(TEST_NAME) *skel,
	...
	unsigned long long p0_r2 = skel->bss->shared.r2[c];
	unsigned long long p1_r4 = skel->bss->shared.r4[c];
	// Check if this iteration matches the exists clause
	states[p0_r2][p1_r4]++;
	if (p0_r2 == 0 && p1_r4 == 0) {
			*matches += 1;
			expected_state_p = &states[p0_r2][p1_r4];
	} else {
			*non_matches += 1;
	}
}
const char* var_names[] = {"p0_r2", "p1_r4"};
