...
void reset_state(struct r_poonceonces_bpf *skel) {
...
int main(int argc, char **argv)
{
	...
	err = r_poonceonces_bpf__load(skel);
	if (err) {
...
prog1_fd < 0 || prog2_fd < 0
	...
	for (int i = 0; i < config.iterations; i++) {
		...
		for (int ii=0; ii<1000; ii++) {
			...
			int var_y = skel->bss->shared.y[ii];
...
p1_r0 == 0 && var_y == 2) {
	...
	// Print test header
	printf("Test R+poonceonces Allowed\n");
	...
	printf("Condition exists (1:r0=0 /\\ y=2) is %s\n", 
		matches > 0 ? "validated" : "NOT validated");
	
	// Print observation summary
	const char* result_type = matches > 0 ? "Sometimes" : "Never";
	printf("Observation R+poonceonces %s %d %d\n", 
		result_type, matches, non_matches);
	printf("Time R+poonceonces %.2f\n\n", elapsed_time);
	
	// Print timestamp
	printf("%s\n", time_str);

cleanup:
	r_poonceonces_bpf__destroy(skel);
	return -err;
}