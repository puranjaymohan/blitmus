/* Auto-generated from S+fencembonceonce+poacquireonce.litmus */
...
#include "s_fencembonceonce_poacquireonce.skel.h"
#define THREADS 2
#define TEST_NAME s_fencembonceonce_poacquireonce
#define TEST_NAME_PRINT "S+fencembonceonce+poacquireonce"
#define EXISTS_CLAUSE "x=2 /\\ 1:r0=1"
...
static void check_cond (STRUCT_NAME(TEST_NAME) *skel,
	...
	unsigned long long var_x = skel->bss->shared.x[c];
	// Check if this iteration matches the exists clause
	states[p1_r0][var_x]++;
	if (p1_r0 == 1 && var_x == 2) {
			*matches += 1;
			expected_state_p = &states[p1_r0][var_x];
	} else {
			*non_matches += 1;
	}
}
const char* var_names[] = {"p1_r0", "var_x"};
