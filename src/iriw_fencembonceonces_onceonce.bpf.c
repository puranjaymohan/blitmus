...
struct {
	...
	volatile __u64 P3_r0[10000];
	volatile __u64 P3_r1[10000];
...
int handle_tp1(void *ctx)
{
		...
		smp_mb();
		shared.P1_r1[i] = READ_ONCE(shared.y[i]);
        }
        smp_mb();
        return 0;
}
...
int handle_tp3(void *ctx)
{
        ...
        bpf_for (i, 0, 10000) {
                barrier_wait(3, i);
                shared.P3_r0[i] = READ_ONCE(shared.y[i]);
		smp_mb();
		shared.P3_r1[i] = READ_ONCE(shared.x[i]);
        }
        smp_mb();
        return 0;
}
